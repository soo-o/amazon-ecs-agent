...
func newTaskManifestHandler(cluster string, containerInstanceArn string, acsClient wsclient.ClientServer,
...
func (*taskManifestHandler) Name() string { return "task manifest responder" }

func (taskManifestHandler *taskManifestHandler) RegisterResponder(respond wsclient.RespondFunc) {
	taskManifestHandler.respond = respond
}

// HandlerFunc unused in this poc due to two different handlers being used for this responder.
// The two handlers below this function were directly referred to and registered separately.
func (taskManifestHandler *taskManifestHandler) HandlerFunc() wsclient.RequestHandler {
	return taskManifestHandler.handleTaskManifestMessage
}

func (taskManifestHandler *taskManifestHandler) taskManifestMessageHandler() wsclient.RequestHandler {
	return taskManifestHandler.handleTaskManifestMessage
}

func (taskManifestHandler *taskManifestHandler) taskStopVerificationAckHandler() wsclient.RequestHandler {
	return taskManifestHandler.handleTaskStopVerificationAck
}

func (taskManifestHandler *taskManifestHandler) handleTaskManifestMessage(message *ecsacs.TaskManifestMessage) error {
...
func (taskManifestHandler *taskManifestHandler) handleTaskStopVerificationAck(message *ecsacs.TaskStopVerificationAck) error {
	if err := taskManifestHandler.handleSingleMessageVerificationAck(message); err != nil {
		errMsg := fmt.Sprintf("Error handling Verification ack with messageID: %s, error: %v", message.MessageId, err)
		seelog.Warnf(errMsg)
...
func (taskManifestHandler *taskManifestHandler) handleTaskManifestSingleMessage(
	...
	if agentLatestSequenceNumber < seqNumberFromMessage {
		...
		*taskManifestHandler.latestSeqNumberTaskManifest = *message.Timeline
		...
		wg.Add(1)
		go func() {
			// Ack the task manifest message.
			defer wg.Done()
			taskManifestHandler.ackTaskManifestMessage(*message.MessageId)
		}()
		...
		go func() {
			...
			if len(tasksToKill) > 0 {
				...
				if err := taskManifestHandler.acsClient.MakeRequest(taskStopVerificationMessage); err != nil {
